  ...
  const activity = useMemo(() => {
    ...
    const sales = (purchaseEvents ?? [])
      ...
      .map(evt => ({
        ...
        xp: ((evt.args.points ?? 0n) * 20n) / 100n,
    ...
    const redemptions = (redeemEvents ?? [])
      ...
      .map(evt => ({
        ...
        // TicketGenerated no tiene un arg de timestamp, pero `expiresAt = block.timestamp +
        // TICKET_TTL` (MichiPoints.sol), asi que restar el TTL recupera el momento del canje
        // sin pedir datos de bloque extra.
